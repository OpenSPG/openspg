...
public final class SchemaCache {
    ...
    public SchemaCache() {
        SPG_TYPE_CACHE = CacheBuilder.newBuilder()
            .maximumSize(1000)
            .expireAfterWrite(3, TimeUnit.MINUTES)
            .build();
        SPG_RELATION_CACHE = CacheBuilder.newBuilder()
            .maximumSize(1000)
            .expireAfterWrite(3, TimeUnit.MINUTES)
            .build();
        SPG_CONCEPT_CACHE = CacheBuilder.newBuilder()
            .maximumSize(100)
            .expireAfterWrite(3, TimeUnit.MINUTES)
            .build();
        SPG_PROJECT_SCHEMA_CACHE = CacheBuilder.newBuilder()
            .maximumSize(50)
            .expireAfterWrite(3, TimeUnit.MINUTES)
            .build();
    }
    ...
    public ApiResponse<Relation> queryRelation(
        String key, Supplier<ApiResponse<Relation>> supplier) {
    ...
    private <T> ApiResponse<T> cacheOrCall(Cache<String, ApiResponse<T>> cache,
        String key, Supplier<ApiResponse<T>> supplier) {
        try {
