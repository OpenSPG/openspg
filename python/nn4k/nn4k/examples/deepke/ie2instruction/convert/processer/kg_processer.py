...
import numpy as np
np.random.seed(42)
from ie2instruction.convert.processer.processer import Processer
...
class KGProcesser(Processer):
    ...
    def get_data_dict(self, datas):
        data_dict = defaultdict(list)
        for data in datas:
            type_set = set()
            for label in data['relation']:
                type_set.add(label['relation'])
            type_set = sorted(list(type_set))
            label_text = ''
            if len(type_set) != 0:
                label_text = '_'.join(type_set)
            data_dict[label_text].append(data)
        return data_dict 
