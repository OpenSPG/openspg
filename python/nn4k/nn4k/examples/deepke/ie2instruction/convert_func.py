...
from ie2instruction.convert.utils.instruction import instruction_mapper
from ie2instruction.convert.utils.utils import stable_hash, write_to_json
from ie2instruction.convert.processer import get_processer
from ie2instruction.convert.converter import get_converter
...
def get_test_data(task, language, source, datas, processer, split_num):
    results = []
    for record in datas:
...
def get_train_data(task, language, source, datas, processer, converter, split_num, random_sort):
    results = []
    for record in datas:
...
def process(options):
    ...
    if options.split == 'test':  
        results = get_test_data(options.task, options.language, options.source, datas, processer, options.split_num) 
    else:
        results = get_train_data(options.task, options.language, options.source, datas, processer, converter, options.split_num, options.random_sort)
...
python ie2instruction/convert_func.py \
    --src_path data/KG/sample.json \
    --tgt_path data/KG/test.json \
    --schema_path data/KG/schema.json \
    --language zh \
    --task KG \
    --split_num 4 \
    --split test
...
if __name__ == "__main__":
    ...
    parse.add_argument("--task", type=str, default="NER", choices=['RE', 'NER', 'EE', 'EET', 'EEA', 'KG'])
