...
from musique_agent import MusiqueAnswerQuestionWithSPO, MusiqueAnswerQuestionWithText, HierarchicalAnswerQuestion, MusiqueDivideAndConquerAgent
...
def create_musique_hierarchical_info_tools(musique_data, get_llm_fn, get_embed_fn, prompt_template_dir, intermediate_dir):
    info_tools = []
    spo_retriver = SPOInfoRetriverWithTripleExtractor(
        musique_dataset=musique_data, 
        llm=get_llm_fn(), 
...
def create_divide_and_conquer_agent_v1(musique_data, get_llm_fn, get_embed_fn, prompt_template_dir, intermediate_dir, debug_mode):
    # spo answer
    llm = get_llm_fn()
    ...
    answer_parent_question = AnswerQuestionWithContext(
        llm_module=llm, 
        use_default_prompt_template=False,
        prompt_template_dir=prompt_template_dir
    )
    
    atom_answer_question = MusiqueAnswerQuestionWithSPO(
        llm_module=llm,
...
def create_divide_and_conquer_agent_v2(musique_data, get_llm_fn, get_embed_fn, prompt_template_dir, intermediate_dir, debug_mode):
    ...
    answer_with_spo = MusiqueAnswerQuestionWithSPO(
        llm_module=llm,
        spo_info_tool=info_tools[0],
        prompt_template_dir=prompt_template_dir,  
    )
    
    answer_with_text = MusiqueAnswerQuestionWithText(
        llm_module=llm,
        text_info_tool=info_tools[1],
        prompt_template_dir=prompt_template_dir,  
    )
    
    atom_answer_question = HierarchicalAnswerQuestion(
        llm_module=llm,
...
def run_experiment_impl(create_agent_fn,
    ...
    if debug_mode:
        ...
        expected_predicted_support_idxs = [q_dict['paragraph_support_idx'] for q_dict in data_line['question_decomposition']]
...
def run_deepseek_divide_and_conquer_experiment():
    ...
    run_experiment_impl(
        create_agent_fn=create_divide_and_conquer_agent_v2,
        get_llm_fn=get_official_deepseek_llm,
        get_embed_fn=get_maya_embed_fn,
        data_dir=data_dir,
