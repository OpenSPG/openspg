...
class TwoDegreeSubGraph(BuilderJob):
    def build(self):
        ...
        event_mapping = (
            SPGTypeMapping(spg_type_name=TEST.CenterEvent, fusing_strategy=FusingStrategyEnum.NewInstance)
        ...
        entity1_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Entity1, fusing_strategy=FusingStrategyEnum.NewInstance)
            .add_property_mapping("entity", TEST.Entity1.id)
            .add_property_mapping("entity", TEST.Entity1.name)
            .add_relation_mapping(
                "entity_relation", TEST.Entity1.entityRelation, TEST.Entity2, LinkingStrategyEnum.IDEquals
            )
            .add_predicting_relation(TEST.Entity1.predictRelation, TEST.Entity3)
        )

        concept1_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Concept1, fusing_strategy=FusingStrategyEnum.NewInstance)
        ...
        entity2_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Entity2, fusing_strategy=FusingStrategyEnum.NewInstance)
            .add_property_mapping("entity_relation", TEST.Entity2.id)
            .add_property_mapping("entity_relation", TEST.Entity2.name)
        )

        entity3_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Entity3, fusing_strategy=FusingStrategyEnum.NewInstance)
            .add_property_mapping("predict_relation", TEST.Entity3.id)
            .add_property_mapping("predict_relation", TEST.Entity3.name)
        )

        concept2_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Concept2, fusing_strategy=FusingStrategyEnum.NewInstance)
            .add_property_mapping("lead_to_concept2", TEST.Concept3.id)
            .add_property_mapping("lead_to_concept2", TEST.Concept3.name)
        )

        concept3_mapping = (
            SPGTypeMapping(spg_type_name=TEST.Concept3, fusing_strategy=FusingStrategyEnum.NewInstance)
