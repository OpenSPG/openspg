...
import com.antgroup.openspg.server.core.schema.model.constraint.EnumConstraint;
import com.antgroup.openspg.server.core.schema.model.constraint.MultiValConstraint;
import com.antgroup.openspg.server.core.schema.model.constraint.NotNullConstraint;
import com.antgroup.openspg.server.core.schema.model.constraint.RangeConstraint;
import com.antgroup.openspg.server.core.schema.model.constraint.RegularConstraint;
import com.antgroup.openspg.server.core.schema.model.constraint.UniqueConstraint;
...
public class ConstraintDOConvertor {
            ...
            v -> {
              ...
              switch (constraintItemEnum) {
                ...
                case RANGE:
                  ...
                  constraintDO.setUpDownBoundary(
                      ConstraintRangeEnum.parse(rangeConstraint).getValue());
                  break;
                default:
  ...
  public static Constraint toConstraint(ConstraintDO constraintDO) {
    ...
    if (YesOrNoEnum.isYes(constraintDO.getIsEnum())) {
      constraintItems.add(
          new EnumConstraint(JSON.parseArray(constraintDO.getEnumValue(), String.class)));
    }
    ...
    // 解析数值型约束条件
    ConstraintRangeEnum constraintRangeEnum =
        ConstraintRangeEnum.getRange(constraintDO.getUpDownBoundary());
    switch (constraintRangeEnum) {
      case GT:
        constraintItems.add(new RangeConstraint(constraintDO.getMinValue(), null, true, null));
        break;
      case GT_OE:
        constraintItems.add(new RangeConstraint(constraintDO.getMinValue(), null, false, null));
        break;
      case LT:
        constraintItems.add(new RangeConstraint(null, constraintDO.getMaxValue(), null, true));
        break;
      case LT_OE:
        constraintItems.add(new RangeConstraint(null, constraintDO.getMaxValue(), null, false));
        break;
      case OPEN:
        constraintItems.add(
            new RangeConstraint(
                constraintDO.getMinValue(), constraintDO.getMaxValue(), true, true));
        break;
      case OAOB:
        constraintItems.add(
            new RangeConstraint(
                constraintDO.getMinValue(), constraintDO.getMaxValue(), true, false));
        break;
      case BCAO:
        constraintItems.add(
            new RangeConstraint(
                constraintDO.getMinValue(), constraintDO.getMaxValue(), false, true));
        break;
      case CLOSED:
        constraintItems.add(
            new RangeConstraint(
                constraintDO.getMinValue(), constraintDO.getMaxValue(), false, false));
        break;
      default:
        break;
    }
