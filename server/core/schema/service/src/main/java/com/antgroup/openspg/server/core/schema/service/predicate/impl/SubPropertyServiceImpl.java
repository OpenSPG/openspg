...
public class SubPropertyServiceImpl implements SubPropertyService {
  ...
  @Autowired private SubPropertyRepository subPropertyRepository;
  ...
  public int update(SubProperty subProperty) {
    if (subProperty.getConstraint() == null) {
      SimpleSubProperty simpleSubProperty =
          subPropertyRepository.queryByUniqueId(subProperty.getUniqueId());
  ...
  public int delete(SubProperty subProperty) {
    if (subProperty.getConstraint() != null) {
      constraintRepository.deleteById(Lists.newArrayList(subProperty.getConstraint().getId()));
    }
  ...
  public List<SubProperty> queryBySubjectId(List<Long> subjectIds, SPGOntologyEnum ontologyEnum) {
    ...
    List<SimpleSubProperty> simpleSubProperties =
        subPropertyRepository.queryBySubjectId(subjectIds, ontologyEnum);
    if (CollectionUtils.isEmpty(simpleSubProperties)) {
      return Collections.emptyList();
    }

    List<Long> constraintIds =
        simpleSubProperties.stream()
            .map(SimpleSubProperty::getConstraintId)
            .filter(e -> e != null && e > 0)
            .collect(Collectors.toList());
    List<Constraint> constraints = constraintRepository.queryById(constraintIds);

    List<PropertyRef> propertyRefs =
        propertyRepository.queryRefByUniqueId(subjectIds, ontologyEnum);

    Set<Long> objectIds =
        simpleSubProperties.stream()
            .map(e -> e.getObjectId().getUniqueId())
            .collect(Collectors.toSet());
    List<SimpleSPGType> objectTypes =
        spgTypeRepository.queryByUniqueId(Lists.newArrayList(objectIds));

    return PredicateAssemble.toSubProperty(
        simpleSubProperties, propertyRefs, objectTypes, constraints);
  }
