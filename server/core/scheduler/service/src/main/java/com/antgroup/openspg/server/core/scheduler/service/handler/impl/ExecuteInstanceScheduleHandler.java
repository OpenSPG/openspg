...
@Slf4j
@Component("executeInstanceScheduleHandler")
public class ExecuteInstanceScheduleHandler implements SchedulerHandler {
  ...
  public SchedulerHandlerResult process(JSONObject jobContext) {
    ...
    try {
      ...
      return new SchedulerHandlerResult(TaskStatus.FINISH, TaskStatus.FINISH.name());
    } catch (Exception e) {
      log.error("run {} Exception", this.getClass().getSimpleName(), e);
      return new SchedulerHandlerResult(TaskStatus.ERROR, ExceptionUtils.getStackTrace(e));
    }
  }
