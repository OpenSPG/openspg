...
public class SchedulerExecuteServiceImpl implements SchedulerExecuteService {
    ...
    private ConcurrentHashMap<String, ThreadPoolExecutor> instanceExecutorMap = new ConcurrentHashMap<>();
    ...
    public void executeInstance(Long id, List<SchedulerTask> tasks) {
        ...
        if (InstanceStatus.isFinish(instance.getStatus())) {
            LOGGER.info(String.format("instanceStatus is FINISH slip id:%s status:%s", id, instance.getStatus()));
            return;
        }
        List<SchedulerTask> processList = tasks.stream().filter(s -> TaskStatus.isRunning(s.getStatus())).collect(Collectors.toList());
    ...
    public void executeNextTask(JobTaskContext context) {
        ...
        for (WorkflowDag.Node nextNode : nextNodes) {
            SchedulerTask nextTask = schedulerTaskService.queryByInstanceIdAndType(instance.getId(), nextNode.getType());
        ...
        if (context.isTaskFinish() && CollectionUtils.isNotEmpty(taskList)) {
            Runnable instanceRunnable = () -> {
                try {
                    ...
                    LOGGER.error(String.format("executeInstance error uniqueId:%s", instance.getUniqueId()), e);
                }
            };
    ...
    private List<SchedulerTask> checkAndUpdateWaitStatus(SchedulerInstance instance, List<SchedulerTask> tasks) {
        List<SchedulerTask> result = Lists.newArrayList();
        List<SchedulerTask> processList = tasks.stream().filter(s -> TaskStatus.WAIT.equals(s.getStatus())).collect(Collectors.toList());
        if (CollectionUtils.isEmpty(processList)) {
            return result;
        }
        WorkflowDag workflowDag = JSON.parseObject(instance.getWorkflowConfig(), WorkflowDag.class);
        processList.forEach(it -> {
            ...
            for (WorkflowDag.Node preNode : preNodes) {
                SchedulerTask preTask = schedulerTaskService.queryByInstanceIdAndType(instance.getId(), preNode.getType());
    ...
    private ThreadPoolExecutor getThreadPoolExecutor(String type, int corePoolSize, int maximumPoolSize) {
        ThreadPoolExecutor taskExecutor = new ThreadPoolExecutor(corePoolSize, maximumPoolSize, 30, TimeUnit.MINUTES,
