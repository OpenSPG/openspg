...
public abstract class JobTaskTemplate implements JobTask {
    ...
    public void executeEntry(JobTaskContext context) {
        ...
        try {
            ...
            LOGGER.warn(String.format("JobProcessTemplate process error uniqueId:%s", context.getInstance().getUniqueId()), e);
        }
    ...
    public void processStatus(JobTaskContext context, TaskStatus status, boolean lock) {
        try {
            ...
            LOGGER.error(String.format("process status error uniqueId:%s", context.getInstance().getUniqueId()), e);
        } finally {
    ...
    private boolean lockTask(JobTaskContext context) {
        ...
        if (now.before(unLockTime)) {
            context.addTraceLog("上次调度执行锁抢锁时间：%s，未超过执行阈值。请等待调度执行完成。", DateTimeUtils.getDate2LongStr(task.getLockTime()));
            return false;
        }
        context.addTraceLog("上次调度执行锁抢锁时间：%s，超过执行阈值。当前流程直接抢占调度锁进行执行。", DateTimeUtils.getDate2LongStr(task.getLockTime()));
    ...
    private void startNextNode(JobTaskContext context, WorkflowDag workflowDag, WorkflowDag.Node nextNode) {
        ...
        for (WorkflowDag.Node preNode : preNodes) {
            ...
            SchedulerTask preTask = schedulerTaskService.queryByInstanceIdAndType(task.getInstanceId(), preNode.getType());
        ...
        SchedulerTask nextTask = schedulerTaskService.queryByInstanceIdAndType(task.getInstanceId(), nextNode.getType());
        if (nextTask != null) {
    ...
    private boolean checkAllNodesFinish(JobTaskContext context) {
        ...
        List<SchedulerTask> taskList = schedulerTaskService.queryByInstanceId(context.getInstance().getId());
    ...
    public void setInstanceFinish(JobTaskContext context, TaskStatus taskStatus, InstanceStatus instanceStatus) {
        SchedulerInstance instance = context.getInstance();
        context.addTraceLog("完成整个流程，后续节点状态将全部变更为：%s。任务状态置为：%s", taskStatus.name(), instanceStatus.name());
