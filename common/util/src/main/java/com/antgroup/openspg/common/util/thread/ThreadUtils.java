...
public class ThreadUtils {
    ...
    /**
     * Wrapper over newFixedThreadPool. Thread names are formatted as prefix-ID, where ID is a unique, sequentially
     * assigned integer.
     */
    ...
    public static ScheduledExecutorService newDaemonSingleThreadScheduledExecutor(String threadName) {
        ThreadFactory threadFactory = new ThreadFactoryBuilder()
            .setDaemon(true).setNameFormat(threadName).build();
    ...
    public static int nThreads(String nThreads) {
        if (nThreads.startsWith("*")) {
            return Runtime.getRuntime().availableProcessors() *
                Integer.parseInt(nThreads.substring(1));
        }
