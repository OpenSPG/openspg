 ...
 *
 * <tt>LPGGraphStoreClient</tt> has a built-in native adapter for converting <tt>SPG</tt> to <tt>LPG</tt>,
 * typically used as {@link GraphStoreClient GraphStoreClient} by the core service layer, exposing two interfaces:
 * {@link GraphStoreClient#alterSchema alterSchema(SPGSchemaAlterCmd)} and
 * {@link GraphStoreClient#manipulateRecord manipulateRecord(SPGRecordManipulateCmd)}
 *
 * <tt>LPGGraphStoreClient</tt> <strong>SHOULD</strong> provide the following services and API interfaces:
 *     <ul>
 *         <li><tt>{@link LPGDataDefinitionService LPGDataDefinitionService}:</tt></li>
 *         &nbsp&nbsp
 *         {@link LPGDataDefinitionService#createVertexType createVertexType(..)},
 *         {@link LPGDataDefinitionService#createEdgeType  createEdgeType(..)},
 *         {@link LPGDataDefinitionService#alterVertexType alterVertexType(..)},
 *         {@link LPGDataDefinitionService#alterEdgeType  alterEdgeType(..)},
 *         {@link LPGDataDefinitionService#dropVertexType dropVertexType(..)},
 *         {@link LPGDataDefinitionService#dropEdgeType  dropEdgeType(..)}
 *
 *         <li><tt>{@link LPGDataManipulationService LPGDataManipulationService}:</tt></li>
 *         &nbsp&nbsp
 *         {@link LPGDataManipulationService#upsertVertex upsertVertex(..)},
 *         {@link LPGDataManipulationService#upsertEdge  upsertEdge(..)},
 *         {@link LPGDataManipulationService#deleteVertex deleteVertex(..)},
 *         {@link LPGDataManipulationService#deleteEdge  deleteEdge(..)}
 *
 *
 *         <li><tt>{@link LPGDataQueryService LPGDataQueryService}:</tt></li>
 *         &nbsp&nbsp
 *         {@link LPGDataQueryService#queryRecord queryRecord(..)}
 *     </ul>
 *     <strong><code>NOTE:</code></strong> in future version, if <tt>LPGGraphStoreClient</tt> additional provides
 *      the following API interface, it will reduce the calls to the client:
 *     <ul>
 *         <li><tt>{@link LPGDataDefinitionService#batchTransactionalSchemaOperations LPGDataDefinitionService.batchTransactionalSchemaOperations(..)}</tt></li>
 *     </ul>
 *
 * </P>
 */
public abstract class BaseLPGGraphStoreClient implements GraphStoreClient,
    LPGDataDefinitionService, LPGDataManipulationService, LPGDataQueryService {
    ...
    public boolean alterSchema(SPGSchemaAlterCmd cmd) {
        ...
        List<BaseLPGSchemaOperation> lpgSchemaOperations = spgSchema2LpgService.translate(cmd.getSpgSchema());
        // Alter LPG schema one by one.
        // TODO in the future: add code switch to batch alter LPG schema transactional if LPG graph store support to
     ...
     *     <ol>
     *         <li>{@link DropEdgeTypeOperation DropEdgeTypeOperation}s</li>
     *         <li>{@link DropVertexTypeOperation DropVertexTypeOperation}s</li>
     *         <li>{@link CreateVertexTypeOperation CreateVertexTypeOperation}s</li>
     *         <li>{@link CreateEdgeTypeOperation CreateEdgeTypeOperation}s</li>
     *         <li>{@link AlterVertexTypeOperation AlterVertexTypeOperation}s</li>
     *         <li>{@link AlterEdgeTypeOperation AlterEdgeTypeOperation}s</li>
     *     </ol>
     * </p>
     *
     * @param lpgSchemaOperations operations for LPG schema.
     * @return <code>true</code> if results of alterations are all successfully,
    ...
    private boolean alterLPGSchemaOneByOne(List<BaseLPGSchemaOperation> lpgSchemaOperations) {
        ...
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.DROP_EDGE_TYPE.equals(record.getOperationTypeEnum()))
            .forEach(record -> dropEdgeType((DropEdgeTypeOperation) record));
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.DROP_VERTEX_TYPE.equals(record.getOperationTypeEnum()))
            .forEach(record -> dropVertexType((DropVertexTypeOperation) record));

        // Do create for LPG type, vertex type first.
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.CREATE_VERTEX_TYPE.equals(record.getOperationTypeEnum()))
            .forEach(record -> createVertexType((CreateVertexTypeOperation) record));
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.CREATE_EDGE_TYPE.equals(record.getOperationTypeEnum()))
            .forEach(record -> createEdgeType((CreateEdgeTypeOperation) record));

        // Do alter for LPG type.
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.ALTER_VERTEX_TYPE.equals(record.getOperationTypeEnum()))
            .forEach(record -> alterVertexType((AlterVertexTypeOperation) record));
        lpgSchemaOperations.stream()
            .filter(record -> VertexEdgeTypeOperationEnum.ALTER_EDGE_TYPE.equals(record.getOperationTypeEnum()))
    ...
    public boolean manipulateRecord(SPGRecordManipulateCmd cmd) {
        List<LPGRecordAlterItem> alterItems = cmd.getAlterItems()
            .stream()
            .flatMap(alterItem ->
                spgRecord2LpgService.convert(alterItem).stream())
     ...
     *     <ol>
     *         <li>DELETE {@link EdgeRecord EdgeRecord}s</li>
     *         <li>DELETE {@link VertexRecord VertexRecord}s</li>
     *         <li>UPSERT {@link VertexRecord VertexRecord}s</li>
     *         <li>UPSERT {@link EdgeRecord EdgeRecord}s</li>
     *     </ol>
     * </p>
     *
    ...
    private boolean manipulateLPGRecord(List<LPGRecordAlterItem> alterItems) {
        try {
            manipulateEdgeRecord(
                getEdgeRecord(alterItems, RecordAlterOperationEnum.DELETE), RecordAlterOperationEnum.DELETE);
            manipulateVertexRecord(
                getVertexRecord(alterItems, RecordAlterOperationEnum.DELETE), RecordAlterOperationEnum.DELETE);

            manipulateVertexRecord(
                getVertexRecord(alterItems, RecordAlterOperationEnum.UPSERT), RecordAlterOperationEnum.UPSERT);
            manipulateEdgeRecord(
                getEdgeRecord(alterItems, RecordAlterOperationEnum.UPSERT), RecordAlterOperationEnum.UPSERT);
            return true;
        } catch (Exception e) {
    ...
    private void manipulateVertexRecord(
        Map<String, List<VertexRecord>> vertexRecordMap, RecordAlterOperationEnum alterOp) throws Exception {
    ...
    private void manipulateEdgeRecord(
        Map<EdgeTypeName, List<EdgeRecord>> edgeRecordMap, RecordAlterOperationEnum alterOp) throws Exception {
