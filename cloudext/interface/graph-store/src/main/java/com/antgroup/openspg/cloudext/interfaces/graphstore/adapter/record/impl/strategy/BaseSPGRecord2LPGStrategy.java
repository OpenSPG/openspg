...
public abstract class BaseSPGRecord2LPGStrategy implements SPGRecord2LPGStrategy {
     ...
     * which should be deleted before adding new semantic edges converted by {@link SPGPropertyRecord}.
     */
    ...
    protected List<LPGRecordAlterItem> normalizeProperties(SPGRecordAlterItem alterItem) {
        ...
        List<LPGRecordAlterItem> resultAlterItems = new ArrayList<>(
            getEdgeRecordsToDelete(advancedRecord, semanticProperties)
        );
        ...
        if (RecordAlterOperationEnum.UPSERT.equals(alterOperationEnum)) {
            ...
            List<SPGPropertyRecord> standardProperties = semanticProperties.stream()
    ...
    private List<LPGRecordAlterItem> getEdgeRecordsToDelete(
        ...
        GraphLPGRecordStruct recordStruct = (GraphLPGRecordStruct)
            lpgGraphStoreClient.queryRecord(new OneHopLPGRecordQuery(
                advancedRecord.getId(),
                advancedRecord.getName(),
                semanticProperties.stream()
                    .map(p -> new EdgeTypeName(
                            advancedRecord.getName(),
                            p.getName(),
    ...
    private List<LPGRecordAlterItem> getVertexRecordsToUpsert(List<SPGPropertyRecord> standardProperties) {
