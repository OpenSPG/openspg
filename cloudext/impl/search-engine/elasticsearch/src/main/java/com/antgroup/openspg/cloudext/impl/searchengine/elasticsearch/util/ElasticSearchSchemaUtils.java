...
public class ElasticSearchSchemaUtils {
    ...
    public static int addNewFieldsIntoIdx(
        ...
        for (IdxSchema idxSchema : idxSchemas) {
            EsMapping mapping = EsMapping.fromIdxMapping(idxSchema.getIdxMapping());
            ForestResponse<String> response = client.addNewFieldsIntoIdx(
                idxSchema.getIdxName(),
                mapping.getProperties()
            );
            if (!response.isSuccess()) {
                throw new RuntimeException("addNewFieldsIntoIdx error, and errorMsg=" + response.getContent());
            }
            updated += 1;
        }
        return updated;
    ...
    public static List<IdxSchema> queryAllIdxSchema(ElasticSearchSchemaClient client) {
        ...
        Map<String, EsIdxDigest> esIdxDigest = JSON.parseObject(
            response.getResult(),
            new TypeReference<Map<String, EsIdxDigest>>() {
            });
    ...
    private static List<IdxSchema> convert2IdxSchema(Map<String, EsIdxDigest> esIdxDigest) {
        ...
        return esIdxDigest.entrySet()
            .stream()
            .map(entry -> new IdxSchema(entry.getKey(), convert2IdxMapping(entry.getValue().getMappings())))
    ...
    private static IdxMapping convert2IdxMapping(EsMapping esMapping) {
        ...
        List<IdxField> idxFields = esMapping.getProperties()
            .entrySet()
            .stream()
            .map(entry -> new IdxField(entry.getKey(), BasicTypeEnum.from(entry.getValue().getType())))
            .collect(Collectors.toList());
