...
import com.antgroup.openspg.cloudext.impl.repository.jdbc.repository.spgschema.convertor.OntologyConvertor;
...
public class SPGTypeRepositoryImpl implements SPGTypeRepository {
    ...
    @Autowired
    private ProjectOntologyRelRepository projectOntologyRelRepository;
    @Autowired
    private OntologyParentRelRepository ontologyParentRelRepository;
    ...
    public int save(SimpleSPGType simpleSpgType) {
        ...
        if (!SPGTypeEnum.BASIC_TYPE.equals(simpleSpgType.getSpgTypeEnum())
            ...
            ProjectOntologyRel projectOntology = new ProjectOntologyRel(
                null,
    ...
    public int delete(SimpleSPGType advancedType) {
        ...
        example.createCriteria()
            .andOriginalIdEqualTo(advancedType.getUniqueId());
    ...
    public List<SimpleSPGType> queryByProject(Long projectId) {
        ...
        List<Long> uniqueIds = rels.stream().map(ProjectOntologyRel::getResourceId).collect(Collectors.toList());
    ...
    public List<SimpleSPGType> queryAllBasicType() {
        ...
        List<OntologyDOWithBLOBs> ontologyEntityDOS = ontologyDOMapper.selectByExampleWithBLOBs(example);
        return OntologyConvertor.toSpgType(ontologyEntityDOS, Collections.emptyList(), Collections.emptyList());
    }
    ...
    public List<SimpleSPGType> queryAllStandardType() {
        ...
        example.createCriteria()
            ...
            .andStatusEqualTo(ValidStatusEnum.VALID.getCode());
        List<OntologyDOWithBLOBs> ontologyEntityDOS = ontologyDOMapper.selectByExampleWithBLOBs(example);
        List<Long> uniqueIds = ontologyEntityDOS.stream()
            .map(OntologyDOWithBLOBs::getOriginalId).collect(Collectors.toList());
        List<ParentTypeInfo> parentTypeInfos = ontologyParentRelRepository.query(uniqueIds);
    ...
    public List<SimpleSPGType> queryByUniqueId(List<Long> uniqueIds) {
        ...
        List<ParentTypeInfo> parentTypeInfos = ontologyParentRelRepository.query(uniqueIds);
        List<ProjectOntologyRel> rels = projectOntologyRelRepository
            .queryByOntologyId(uniqueIds, SPGOntologyEnum.TYPE);
    ...
    public SimpleSPGType queryByName(String uniqueName) {
        ...
        ProjectOntologyRel projectOntologyRel = projectOntologyRelRepository
            .queryByOntologyId(ontologyDO.getOriginalId(), SPGOntologyEnum.TYPE);
