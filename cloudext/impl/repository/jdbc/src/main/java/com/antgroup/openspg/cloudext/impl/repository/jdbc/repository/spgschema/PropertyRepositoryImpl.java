...
public class PropertyRepositoryImpl implements PropertyRepository {
    ...
    @Override
    public List<SimpleProperty> queryBySubjectId(List<Long> subjectIds, SPGOntologyEnum ontologyEnum) {
        OntologyPropertyDOExample example = new OntologyPropertyDOExample();
        example.createCriteria()
            ...
            .andPropertyCategoryEqualTo(SPGOntologyEnum.PROPERTY.equals(ontologyEnum)
                ? PropertyCategoryEnum.BASIC.name() : PropertyCategoryEnum.ADVANCED.name());
        ...
        return CollectionsUtils.listMap(ontologyPropertyDOS, SimplePropertyConvertor::toSimplePredicate);
    }
    ...
    public Long queryUniqueIdByPO(String predicateName, String objectTypeName) {
        ...
        List<OntologyPropertyDO> ontologyPropertyDOS = ontologyPropertyDOMapper.selectByExampleWithBLOBs(example);
        return CollectionUtils.isEmpty(ontologyPropertyDOS) ? null : ontologyPropertyDOS.get(0).getOriginalId();
    }
    ...
    public List<PropertyRef> queryRefByUniqueId(List<Long> uniqueIds, SPGOntologyEnum ontologyEnum) {
        ...
        simpleProperties.forEach(e -> {
            spgTypeIds.add(e.getSubjectTypeId().getUniqueId());
            spgTypeIds.add(e.getObjectTypeId().getUniqueId());
        });

        List<SimpleSPGType> spgTypes = spgTypeRepository.queryByUniqueId(Lists.newArrayList(spgTypeIds));
        ...
        for (SimpleProperty simpleProperty : simpleProperties) {
            ...
            propertyRefs.add(new PropertyRef(
                subjectType.toRef(),
