...
public class TuGraphSchemaUtils {
    ...
    public static <T extends BaseCreateTypeOperation> boolean createLabel(
        TuGraphDbRpcClient client, String graphName,
        T createOperationRecord, Double timeout) throws Exception {
        ...
        switch (createOperationRecord.getOperationTypeEnum()) {
            case CREATE_EDGE_TYPE:
                cypher = CreateLabelProcedure.of((CreateEdgeTypeOperation) createOperationRecord).getCypher();
                break;
            case CREATE_VERTEX_TYPE:
                cypher = CreateLabelProcedure.of((CreateVertexTypeOperation) createOperationRecord).getCypher();
                break;
            default:
                throw new RuntimeException("unexpected operation type when creating label: "
    ...
    public static boolean addVertexIndexes(
        TuGraphDbRpcClient client, String graphName, String labelName,
        List<CreateIndexOperation> createIndexOperations, Double timeout) throws Exception {
    ...
    public static <T extends BaseAlterTypeOperation> boolean alterLabelAddFields(
        TuGraphDbRpcClient client, String graphName,
        T alterOperationRecord, Double timeout) throws Exception {
        ...
        switch (alterOperationRecord.getOperationTypeEnum()) {
            case ALTER_EDGE_TYPE:
                cypher = AlterLabelAddFieldsProcedure.of((AlterEdgeTypeOperation) alterOperationRecord).getCypher();
                break;
            case ALTER_VERTEX_TYPE:
                cypher = AlterLabelAddFieldsProcedure.of((AlterVertexTypeOperation) alterOperationRecord).getCypher();
                break;
            default:
                throw new RuntimeException("unexpected operation type when alter label add fields: "
    ...
    public static <T extends BaseAlterTypeOperation> boolean alterLabelDelFields(
        ...
        switch (alterOperationRecord.getOperationTypeEnum()) {
            case ALTER_EDGE_TYPE:
                cypher = AlterLabelDelFieldsProcedure.of((AlterEdgeTypeOperation) alterOperationRecord).getCypher();
                break;
            case ALTER_VERTEX_TYPE:
                cypher = AlterLabelDelFieldsProcedure.of((AlterVertexTypeOperation) alterOperationRecord).getCypher();
                break;
            default:
                throw new RuntimeException("unexpected operation type when alter label del fields: "
                    + alterOperationRecord.getOperationTypeEnum());
        }
        client.callCypher(cypher, graphName, timeout);
        return true;
    ...
    public static <T extends BaseLPGSchemaOperation> boolean deleteLabel(
        TuGraphDbRpcClient client, String graphName,
        T operationRecord, Double timeout) throws Exception {
        ...
        switch (operationRecord.getOperationTypeEnum()) {
            ...
            case DROP_VERTEX_TYPE:
                ...
                break;
            default:
                throw new RuntimeException("unexpected operation type when deleting label: "
    ...
    private static boolean tryToDeleteNotExistLabel(
        String dropLabelName, String errorMsg) {
    ...
    public static List<String> getSchemaLabels(
        TypeEnum dataTypeEnum, TuGraphDbRpcClient client, String graphName, Double timeout) throws Exception {
        String cypher = QueryLabelsProcedure.of(dataTypeEnum).getCypher();
        String labelsJsonStr = client.callCypher(
            cypher, graphName, timeout);
        List<QueryLabelsResult> results = JSON.parseObject(
            labelsJsonStr,
            new TypeReference<List<QueryLabelsResult>>() {
            });
        return TuGraphSchemaConvertor.toLabels(results);
    }

    public static VertexType getVertexSchemaByLabel(
        String labelName, TuGraphDbRpcClient client, String graphName, Double timeout) throws Exception {
        ...
        List<GetVertexSchemaResult> results = JSON.parseObject(
            vertexSchemaJsonStr,
            new TypeReference<List<GetVertexSchemaResult>>() {
            });
    ...
    public static EdgeType getEdgeSchemaByLabel(
        ...
        String edgeSchemaJsonStr = client.callCypher(
            cypher, graphName, timeout);
        List<GetEdgeSchemaResult> results = JSON.parseObject(
            edgeSchemaJsonStr,
            new TypeReference<List<GetEdgeSchemaResult>>() {
            });
        if (CollectionUtils.isEmpty(results)) {
            return null;
        }
