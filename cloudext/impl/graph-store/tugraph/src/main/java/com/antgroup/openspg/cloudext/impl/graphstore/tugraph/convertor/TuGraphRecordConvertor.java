...
public class TuGraphRecordConvertor {
  ...
  public static TableLPGRecordStruct toTable(
      String gqlResult, LPGTypeNameConvertor typeNameConvertor) {
  ...
  private static List<List<Object>> toCells(
      List<String> columnNames, JSONArray queryResults, LPGTypeNameConvertor typeNameConvertor) {
    if (CollectionUtils.isEmpty(columnNames)) {
      return new ArrayList<>(0);
    }
  ...
  private static EdgeRecord toEdgeRecord(
      JSONObject jsonObject, LPGTypeNameConvertor typeNameConvertor) {
    EdgeTypeName edgeName =
        typeNameConvertor.restoreEdgeTypeName(jsonObject.getString(TuGraphConstants.KEY_LABEL));
    ...
    for (String key : properties.keySet()) {
       ...
       * *
       */
      if (!EdgeType.SRC_ID.equals(key)
          && !EdgeType.DST_ID.equals(key)
          && !EdgeType.VERSION.equals(key)
          && !TuGraphConstants.TUGRAPH_EDGE_INTERNAL_ID.equals(key)) {
  ...
  private static VertexRecord toVertexRecord(
      JSONObject jsonObject, LPGTypeNameConvertor typeNameConvertor) {
    String type =
        typeNameConvertor.restoreVertexTypeName(jsonObject.getString(TuGraphConstants.KEY_LABEL));
    JSONObject properties = jsonObject.getJSONObject(TuGraphConstants.KEY_PROPERTIES);
  ...
  public static Map<String, Object> toUpsertTuGraphVertices(
    ...
    List<Map<String, Object>> recordParams =
  ...
  public static Map<String, Object> toUpsertTuGraphEdges(
      EdgeTypeName edgeType, List<EdgeRecord> edgeRecords) {
    ...
    List<Map<String, Object>> edgeRecordList =
        edgeRecords.stream()
            .filter(Objects::nonNull)
            .map(
                edgeRecord -> {
