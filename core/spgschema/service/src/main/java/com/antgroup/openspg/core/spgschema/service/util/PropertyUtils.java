...
public class PropertyUtils {
    public static Relation generateSemanticRelation(Property property, Set<SPGTypeIdentifier> spreadStdTypeNames) {
    ...
    public static Property newProperty(SPGTypeRef subjectTypeRef, SPGTypeRef objectTypeRef,
        BasicInfo<PredicateIdentifier> basicInfo, boolean multiValue) {
        Property property = new Property(basicInfo, subjectTypeRef, objectTypeRef,
            false, new PropertyAdvancedConfig());
     ...
     */
    public static Relation newRelation(SPGTypeRef subjectTypeRef,
        SPGTypeRef objectTypeRef,
        BasicInfo<PredicateIdentifier> basicInfo) {
        Relation relation = new Relation(basicInfo, subjectTypeRef,
            objectTypeRef, false, new PropertyAdvancedConfig());
     ...
     */
    public static Property newProperty(SPGTypeRef subjectTypeRef,
        BuiltInPropertyEnum defaultPropertyEnum) {
        BasicInfo<PredicateIdentifier> basicInfo = new BasicInfo<>(
            new PredicateIdentifier(defaultPropertyEnum.getName()),
            defaultPropertyEnum.getNameZh(), defaultPropertyEnum.getDesc());
        SPGTypeRef objectTypeRef = new SPGTypeRef(
        ...
        advancedConfig.setPropertyGroup(PropertyGroupEnum.toEnum(defaultPropertyEnum.getPropertyGroup()));

        Property property = new Property(basicInfo, subjectTypeRef,
            objectTypeRef, false, advancedConfig);
        property.setProjectId(subjectTypeRef.getProjectId());
        return property;
    }
    ...
    public static Relation generateRelation(Property property) {
        ...
        if (property.getSubjectTypeRef().isEventType()) {
            if (property.isSubjectProperty()) {
                basicInfo = new BasicInfo<>(
                    ...
                    basicInfo.getNameZh(), basicInfo.getDesc());
        ...
        Relation newRelation = new Relation(
            ...
            PropertyUtils.copyAdvancedConfig(property.toRef(),
                property.getAdvancedConfig()), true);
     ...
     */
    public static Property inheritProperty(SPGTypeRef subjectTypeRef,
        Property property) {
        PropertyRef propertyRef = new PropertyRef(subjectTypeRef,
            property.getBasicInfo(), property.getObjectTypeRef(),
            SPGOntologyEnum.PROPERTY,
            property.getProjectId(), property.getOntologyId());
        Property newProp = new Property(property.getBasicInfo(), subjectTypeRef,
            property.getObjectTypeRef(), true,
     ...
     */
    public static PropertyAdvancedConfig copyAdvancedConfig(PropertyRef propertyRef,
        ...
        if (CollectionUtils.isNotEmpty(advancedConfig.getSubProperties())) {
            config.setSubProperties(advancedConfig.getSubProperties().stream()
                .map(e -> copySubProperty(propertyRef, e))
                .collect(Collectors.toList()));
        }
        if (CollectionUtils.isNotEmpty(advancedConfig.getSemantics())) {
            config.setSemantics(advancedConfig.getSemantics().stream()
    ...
    private static SubProperty copySubProperty(PropertyRef subjectTypeRef,
        SubProperty subProperty) {
        SubProperty newSubProperty = new SubProperty(subProperty.getBasicInfo(),
            subjectTypeRef, subProperty.getObjectTypeRef(), subProperty.getAdvancedConfig());
    ...
    private static PredicateSemantic copyPredicateSemantic(PropertyRef subjectTypeRef,
        PredicateSemantic semantic) {
        PredicateSemantic newSemantic = new PredicateSemantic(subjectTypeRef,
            semantic.getPredicateIdentifier(), semantic.getObjectTypeRef());
    ...
    private static boolean withIndex(PropertyRef propertyRef) {
        return SchemaConstants.CONCEPT_NAME_PROPERTY_NAME
            .equals(propertyRef.getName())
