...
/**
 * A container that holds three map, a map contains all spg types, a map contains all property types, and a map contains
 * all relation types. it's easy to find a spg type, a property type, or a relation type through the container.
 */
        ...
        if (CollectionUtils.isNotEmpty(baseSpgType.getProperties())) {
            for (Property property : baseSpgType.getProperties()) {
                ...
                if (propertyMap.containsKey(tripleIdentifier)) {
                    throw new IllegalArgumentException(
                        "exist same name property: " + tripleIdentifier);
                }
        ...
        if (CollectionUtils.isNotEmpty(baseSpgType.getRelations())) {
            for (Relation relation : baseSpgType.getRelations()) {
                ...
                if (relationMap.containsKey(tripleIdentifier)) {
                    throw new IllegalArgumentException(
                        "exist same name relation: " + tripleIdentifier);
                }
