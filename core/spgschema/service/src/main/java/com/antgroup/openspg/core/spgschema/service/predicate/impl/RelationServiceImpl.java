...
public class RelationServiceImpl implements RelationService {
    ...
    public List<Relation> queryBySubjectId(List<Long> subjectIds) {
        ...
        List<SimpleProperty> simplePredicates = propertyRepository.queryBySubjectId(
            subjectIds,
        ...
        spgTypeIds.addAll(simplePredicates.stream()
        ...
        List<Long> relationIds = simplePredicates.stream()
            .map(SimpleProperty::getUniqueId).collect(Collectors.toList());
        List<SubProperty> subProperties = subPropertyService
            .queryBySubjectId(relationIds, SPGOntologyEnum.RELATION);

        List<PredicateSemantic> semantics = semanticService.queryBySubjectIds(
            relationIds,
            SPGOntologyEnum.RELATION);

        List<RuleCode> ruleCodes = simplePredicates.stream().map(SimpleProperty::getRuleCode)
            .filter(Objects::nonNull).collect(Collectors.toList());
        List<LogicalRule> logicalRules = logicalRuleService.queryByRuleCode(ruleCodes);

        return PredicateAssemble.toRelation(simplePredicates, spgTypes, subProperties, semantics, logicalRules);
    }
}
