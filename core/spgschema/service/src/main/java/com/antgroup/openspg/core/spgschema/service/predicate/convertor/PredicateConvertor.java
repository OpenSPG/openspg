...
public class PredicateConvertor {
    ...
    public static SimpleProperty toSimpleProperty(Property property) {
        ...
        Long constraintId = property.getConstraint() == null
            ? null : property.getConstraint().getId();
        RuleCode ruleCode = property.getLogicalRule() == null
            ? null : property.getLogicalRule().getCode();
        SPGOntologyEnum ontologyEnum = property instanceof Relation
            ? SPGOntologyEnum.RELATION : SPGOntologyEnum.PROPERTY;

        SimpleProperty simpleProperty = new SimpleProperty(property.getBasicInfo(),
            subjectOntologyId, objectOntologyId,
            property.getObjectTypeRef().getSpgTypeEnum(),
            multiVersionConfig, mountedConceptConfig, encryptTypeEnum,
            propertyGroup, constraintId, ruleCode, ontologyEnum);
    ...
    public static SimpleSubProperty toSimpleSubProperty(SubProperty subProperty) {
        Long constraintId = subProperty.getConstraint() == null
            ? null : subProperty.getConstraint().getId();
        SimpleSubProperty simpleSubProperty = new SimpleSubProperty(
