...
public class OntologyIdHandler {
    ...
    public void handle(SchemaAlterContext context) {
        BaseSPGType rootType = spgTypeService.querySPGTypeByIdentifier(
        ...
        AlterInfoWrap alterInfoWrap = this.buildWrap(context.getReleasedSchema(),
            context.getAlterSchema(), rootType);
    ...
    private AlterInfoWrap buildWrap(
        List<BaseSPGType> onlineTypes, List<BaseAdvancedType> alterTypes,
        BaseSPGType rootThing) {
    ...
    private List<BaseAdvancedType> getSortedTypes(List<BaseSPGType> newSchema) {
        ...
        return sortedAdvancedTypes.stream().filter(e -> e.getAlterOperation() != null).collect(Collectors.toList());
    }
    ...
    private void setPropertyId(BaseAdvancedType advancedType,
    ...
    private void setRelationId(BaseAdvancedType advancedType, Map<SPGTypeIdentifier, BaseSPGType> spgTypeMap) {
    ...
    private void setOntologyId(BaseAdvancedType advancedType, Map<SPGTypeIdentifier,
        BaseSPGType> spgTypeMap, Property property) {
    ...
    private void setSubPropertyId(Property property, Map<SPGTypeIdentifier, BaseSPGType> spgTypeMap) {
        ...
        for (SubProperty subProperty : property.getSubProperties()) {
            ...
            SPGTypeIdentifier objectTypeIdentifier = subProperty.getObjectTypeRef().getBaseSpgIdentifier();
            BaseSPGType objectType = spgTypeMap.get(objectTypeIdentifier);
            if (null == objectType) {
                throw SchemaException.spgTypeNotExist(objectTypeIdentifier.toString());
            }
    ...
    private void setPredicateSemanticId(Property property, Map<SPGTripleIdentifier, Property> propertyMap) {
        if (null == property.getAlterOperation()
    ...
    private void setPredicateSemanticId(Relation relation, Map<SPGTripleIdentifier, Relation> relationMap) {
        if (null == relation.getAlterOperation()
            || Boolean.TRUE.equals(relation.getInherited())
            || Boolean.TRUE.equals(relation.isSemanticRelation())
    ...
    private void setParentTypeInfo(BaseAdvancedType advancedType, Map<SPGTypeIdentifier, BaseSPGType> schemaTypeMap) {
        ...
        inheritPath.add(
            schemaTypeMap.get(SPGTypeIdentifier.parse(SchemaConstants.ROOT_TYPE_UNIQUE_NAME)).getUniqueId());

        Collections.reverse(inheritPath);

        advancedType.setParentTypeInfo(
            parentTypeInfo.withNewParentInfo(
                advancedType.getUniqueId(),
                inheritPath.get(inheritPath.size() - 2),
                inheritPath
            )
        );
    }
