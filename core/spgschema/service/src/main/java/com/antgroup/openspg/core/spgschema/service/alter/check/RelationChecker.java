...
public class RelationChecker extends PropertyChecker {
        ...
        for (Relation relation : relations) {
            ...
            if (names.contains(name)) {
                throw new IllegalArgumentException(String.format(
                    "relation name: %s is duplicated", name)
                );
            }
            names.add(name);
        }
    ...
    private void checkContainExistRelation(BaseAdvancedType advancedType, SchemaCheckContext context) {
        List<Relation> existRelations = context.getExitType(advancedType.getBaseSpgIdentifier()).getRelations();
        ...
        for (Relation existRelation : existRelations) {
            if (!Boolean.TRUE.equals(existRelation.isSemanticRelation())
                ...
                throw new IllegalArgumentException(String.format(
                    "missing relation: %s", existRelation.getSpgTripleName())
                );
            }
        }
    }
