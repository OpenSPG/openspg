...
/**
 * Check the information of property type in spg type is legal, the list of property in spg type should contain all
 * properties that existed, and the name of property type need compliant with agreed format.
 */
public class PropertyChecker {
    ...
    protected void checkBasicInfo(
        ...
        Property property,
        SchemaCheckContext context) {
        if (null == property.getBasicInfo()) {
            throw new IllegalArgumentException(String.format(
                "property/relation:%s basic info can not be null",
                spgTypeIdentifier)
            );
        }
        if (null == property.getName()) {
            throw new IllegalArgumentException(String.format(
                "property/relation:%s name can not be null",
                spgTypeIdentifier)
            );
        }
        if (null == property.getBasicInfo().getNameZh()) {
            throw new IllegalArgumentException(String.format(
                "property/relation: %s nameZh can not be null",
                property.getBasicInfo().getNameZh())
            );
        }
        ...
        if (!context.containSpgType(property.getObjectTypeRef().getBaseSpgIdentifier())) {
            throw new IllegalArgumentException(String.format(
                ...
                property.getBasicInfo().getName(),
                property.getObjectTypeRef().getName()
            ));
        }
    ...
    private void checkLogicalProperty(
        List<Property> logicalProperties,
        SchemaCheckContext context) {
    ...
    protected void checkDSL(String dsl, Catalog catalog) {
        try {
            ...
            LogicalPlannerContext context = new LogicalPlannerContext(
                catalog, parser, new HashMap<>());
    ...
    protected void checkBuiltInProperty(SPGTypeEnum spgTypeEnum, Property property) {
        ...
        if (builtInPropertyNames.contains(property.getName())) {
            throw new IllegalArgumentException(String.format(
                "property: %s is defined by system, no need to define or alter",
                property.getName()
            ));
        }
    }
    ...
    private void checkConstraint(Property property) {
        if (property.getObjectTypeRef().isBasicType()
            ...
            CommonChecker.containForbiddenConstraintType(property.getConstraint(), ConstraintTypeEnum.MULTI_VALUE);
        }
    }
    ...
    private void checkNameDuplicated(List<Property> propertyList) {
        ...
        for (Property property : propertyList) {
            ...
            if (names.contains(name)) {
                throw new IllegalArgumentException(String.format(
                    "property name: %s is duplicated", name
                ));
            }
            names.add(name);
        }
    ...
    private void checkContainExistProp(
        BaseAdvancedType advancedType,
        SchemaCheckContext context) {
        List<Property> existProperties = context.getExitType(
            advancedType.getBaseSpgIdentifier()).getProperties();
        ...
        for (Property existProp : existProperties) {
            if (!names.contains(existProp.getName())) {
                throw new IllegalArgumentException(String.format(
                    "missing property: %s", existProp.getSpgTripleName()
                ));
            }
        }
    }
