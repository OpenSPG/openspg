...
/**
 * Schema meta-information check context, holds all information that used during schema checking, including project
 * information, effective schema information, this changed schema information, and Thing type default properties.
 */
public final class SchemaCheckContext extends BaseToString {
    ...
    public static SchemaCheckContext build(
        Project project,
        List<BaseSPGType> onlineSchema,
        List<BaseAdvancedType> alterSchema) {
        ...
        return new SchemaCheckContext(project, onlineSchema, alterSchema,
            onlineSchemaMap, alterSchemaMap);
    }
    ...
    public List<BaseSPGType> getMergedSchema() {
        Set<SPGTypeIdentifier> spreadableStdTypeNames = Streams.concat(onlineTypes.stream(), alterTypes.stream())
            .filter(e -> !e.isDelete() && SchemaCheckContext.isSpreadableStdType(e))
            .map(WithBasicInfo::getBaseSpgIdentifier).collect(Collectors.toSet());

        List<BaseSPGType> newSchema = new ArrayList<>();
        Map<SPGTypeIdentifier, BaseSPGType> alterMap = alterTypes.stream()
        ...
        for (BaseSPGType spgType : onlineTypes) {
            if (!alterMap.containsKey(spgType.getBaseSpgIdentifier())) {
                ...
                newSchema.add(this.getAlterType(alterMap.get(spgType.getBaseSpgIdentifier()), spreadableStdTypeNames));
            }
        }
    ...
    private BaseSPGType getAlterType(BaseSPGType spgType, Set<SPGTypeIdentifier> spreadableStdTypeNames) {
        ...
        Set<SPGTripleIdentifier> relationIdentifiers = relations.stream()
            .map(Property::getSpgTripleName).collect(Collectors.toSet());
    ...
    private static boolean isSemanticProperty(Property property, Set<SPGTypeIdentifier> spreadableStdTypeNames) {
