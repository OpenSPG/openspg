...
public class PropertyServiceImpl implements PropertyService {
    ...
    @Autowired
    private PropertyRepository simplePredicateRepository;
    @Autowired
    private PropertyAdvancedConfigHandler propertyAdvancedConfigService;
    @Autowired
    ...
    @Autowired
    private SPGTypeRepository simpleSpgTypeRepository;
    ...
    public List<Property> queryBySubjectId(List<Long> subjectIds) {
        ...
        List<SimpleProperty> simpleProperties = simplePredicateRepository.queryBySubjectId(
            subjectIds,
        ...
        spgTypeIds.addAll(simpleProperties.stream()
            .map(e -> e.getSubjectTypeId().getUniqueId()).collect(Collectors.toList()));
        spgTypeIds.addAll(simpleProperties.stream()
            .map(e -> e.getObjectTypeId().getUniqueId()).collect(Collectors.toList()));
        List<SimpleSPGType> spgTypes = simpleSpgTypeRepository.queryByUniqueId(Lists.newArrayList(spgTypeIds));

        List<Long> constraintIds = simpleProperties.stream().map(SimpleProperty::getConstraintId)
            .filter(e -> e != null && e > 0).collect(Collectors.toList());
        List<Constraint> constraints = constraintRepository.queryById(constraintIds);

        List<Long> propertyIds = simpleProperties.stream()
            .map(SimpleProperty::getUniqueId).collect(Collectors.toList());
        List<SubProperty> subProperties = subPropertyService
            .queryBySubjectId(propertyIds, SPGOntologyEnum.PROPERTY);

        List<PredicateSemantic> semantics = semanticService.queryBySubjectIds(
            propertyIds,
            SPGOntologyEnum.PROPERTY);

        List<RuleCode> ruleCodes = simpleProperties.stream().map(SimpleProperty::getRuleCode)
            .filter(Objects::nonNull).collect(Collectors.toList());
        List<LogicalRule> logicalRules = logicalRuleService.queryByRuleCode(ruleCodes);

        return PredicateAssemble.toProperty(simpleProperties, spgTypes,
            constraints, subProperties, semantics, logicalRules);
    }
}
