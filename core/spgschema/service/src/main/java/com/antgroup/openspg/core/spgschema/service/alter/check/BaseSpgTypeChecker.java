...
/**
 * The base class of spg type checker, defines the entry method {@code check} for checking whether a type structure is
 * legal and the implementation of the main process. Each spg type such as StandardType, EntityType, ConceptType and
 * EventType needs to inherit this class and implement the abstract method to verify their unique attributes and logic
 * of each type.
 */
public abstract class BaseSpgTypeChecker {
    ...
    private void checkBasicInfo(BaseAdvancedType advancedType, SchemaCheckContext context) {
        ...
        if (advancedType.isStandardType() && !StandardType.STD_NAMESPACE
            .equals(spgTypeIdentifier.getNamespace())) {
            throw new IllegalArgumentException(String.format(
                "namespace of standard type must be: %s",
                StandardType.STD_NAMESPACE));
        }
        ...
        if (!capitalizeEntityTypeName(spgTypeIdentifier.getNameEn())) {
            throw new IllegalArgumentException(String.format(
                "type name:%s not match: %s",
                spgTypeIdentifier, CAPITALIZE_NAME_REG_EXP));
        }
        if (StringUtils.isBlank(basicInfo.getNameZh())) {
            throw new IllegalArgumentException(String.format(
                "nameZh of type: %s can not be blank", spgTypeIdentifier));
        }
    ...
    private void checkParentInfo(BaseAdvancedType advancedType, SchemaCheckContext context) {
        if (null == advancedType.getParentTypeInfo()
            || null == advancedType.getParentTypeName()) {
            throw new IllegalArgumentException(String.format(
                "parent type of: %s can not be null",
                advancedType.getBaseSpgIdentifier()));
        }

        if (!advancedType.isDelete()
            && !context.containSpgType(advancedType.getParentTypeName())) {
            throw new IllegalArgumentException(String.format(
                "parent type: %s not exist",
                advancedType.getParentTypeName()));
        }
    }
    ...
    private void setPropertyAlterOperation(BaseAdvancedType advancedType) {
        ...
        advancedType.getProperties().forEach(property -> {
            ...
            if (CollectionUtils.isNotEmpty(property.getSubProperties())) {
                property.getSubProperties().forEach(subProperty -> {
            ...
            if (CollectionUtils.isNotEmpty(property.getSemantics())) {
                property.getSemantics().forEach(semantic -> {
                    if (!property.isUpdate()) {
    ...
    private void setRelationAlterOperation(BaseAdvancedType advancedType) {
        ...
        advancedType.getRelations().forEach(relation -> {
            ...
            if (CollectionUtils.isNotEmpty(relation.getSubProperties())) {
                relation.getSubProperties().forEach(subProperty -> {
            ...
            if (CollectionUtils.isNotEmpty(relation.getAdvancedConfig().getSemantics())) {
                relation.getAdvancedConfig().getSemantics().forEach(semantic -> {
                    if (!relation.isUpdate()) {
     ...
     * @param context      the information during check
     */
    public abstract void checkAdvancedConfig(BaseAdvancedType advancedType, SchemaCheckContext context);
}
