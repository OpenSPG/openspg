...
public class LogicalPlan implements Serializable {
    ...
    public static LogicalPlan parse(Pipeline pipeline) {
        ImmutableGraph.Builder<BaseNode<?>> immutable = GraphBuilder.directed()
    ...
    private static BaseNode<?> parse(Node node) {
        switch (node.getType()) {
            case CSV_SOURCE:
                return new CsvSourceNode(node.getId(), node.getName(),
                    (CsvSourceNodeConfig) node.getNodeConfig());
            case EXTRACT:
                return new ExtractNode(node.getId(), node.getName(),
                    (ExtractNodeConfig) node.getNodeConfig());
            case MAPPING:
                return new MappingNode(node.getId(), node.getName(),
                    (MappingNodeConfig) node.getNodeConfig());
            case GRAPH_SINK:
                return new GraphStoreSinkNode(node.getId(), node.getName(),
