...
public class PhysicalPlan implements Serializable {
    ...
    public static PhysicalPlan plan(LogicalPlan logicalPlan) {
        ImmutableGraph.Builder<BasePhysicalNode> immutable = GraphBuilder.directed()
    ...
    private static BasePhysicalNode parse(BaseNode<?> node) {
        switch (node.getType()) {
            case CSV_SOURCE:
                return new CsvFileSourceReader(node.getId(), node.getName(),
                    (CsvSourceNodeConfig) node.getNodeConfig()
                );
            case EXTRACT:
                return new ExtractProcessor(node.getId(), node.getName(),
                    (ExtractNodeConfig) node.getNodeConfig()
                );
            case MAPPING:
                return new MappingProcessor(node.getId(), node.getName(),
                    (MappingNodeConfig) node.getNodeConfig()
                );
            case GRAPH_SINK:
                return new GraphStoreSinkWriter(node.getId(), node.getName(),
                    (GraphStoreSinkNodeConfig) node.getNodeConfig()
                );
            default:
