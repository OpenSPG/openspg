...
public class OperatorInvokerImpl implements OperatorInvoker {
    ...
    public List<BuilderRecord> invoke(BuilderRecord builderRecord, OperatorConfig operatorConfig) {
        ...
        Map<String, Object> result = (Map<String, Object>) operatorFactory
            .invoke(operatorKey, inputVertex.toMap());
        evalResult = mapper.convertValue(result,
            new TypeReference<EvalResult<List<Vertex>>>() {
            });
    ...
    @Override
    public BaseAdvancedRecord invoke(BaseAdvancedRecord baseSpgRecord, Map<String, OperatorConfig> operatorConfigs) {
        ...
        for (SPGPropertyRecord propertyRecord : baseSpgRecord.getSpgProperties()) {
            ...
            if (operatorConfig != null && !SPGRecordTypeEnum.RELATION.equals(baseSpgRecord.getRecordType())) {
                // If an operator is configured on an attribute, the operator will be executed in the future.
                OperatorKey operatorKey = operatorConfig.toKey();
                ...
                switch (operatorConfig.getOperatorType()) {
                    case ENTITY_LINK:
                        ...
                        for (String rawValue : rawValues) {
                            Map<String, Object> result = (Map<String, Object>) operatorFactory
                                .invoke(operatorKey, rawValue, vertex.toMap());
                            EvalResult<List<Vertex>> evalResult1 = mapper.convertValue(result,
                                new TypeReference<EvalResult<List<Vertex>>>() {
                                });
                            ...
                            evalResult1.getData().stream().map(Vertex::getBizId)
                                .filter(Objects::nonNull).forEach(bizIds::add);
                        }
                    ...
                    case PROPERTY_NORMALIZE:
                        ...
                        for (String rawValue : rawValues) {
                            Map<String, Object> stdValue = (Map<String, Object>) operatorFactory
                                .invoke(operatorKey, rawValue, vertex.toMap());
                            EvalResult<String> evalResult2 = mapper.convertValue(stdValue,
                                new TypeReference<EvalResult<String>>() {
                                });
                        ...
                        break;
                    default:
                        throw new IllegalArgumentException("illegal operatorType=" + operatorConfig.getOperatorType());
                }
            }
        }
