...
public class PythonOperatorFactory implements OperatorFactory {
    ...
    public final static String PYTHON_EXEC = "pythonExec";
    public final static String PYTHON_PATHS = "pythonPaths";
    ...
    private void loadOperatorObject(OperatorKey key, OperatorConfig config) {
        ...
        String pythonFileName = jarAddress.substring(
            jarAddress.lastIndexOf("/") + 1, jarAddress.lastIndexOf(".py")
        );
        pythonInterpreter.exec("from objectStore.operator import " + pythonFileName);
        String paramMd5 = Md5Utils.md5Of(
            String.valueOf(Thread.currentThread().hashCode()),
            config.getParams() == null ? "" : config.getParams().toString()
        );
        String operatorObjectName = String.join("_",
            config.toKey().toString(), paramMd5);
        String classNamePath = config.getName().concat("_").concat("v")
            .concat(String.valueOf(config.getVersion())).concat(".")
            .concat(config.getName());
        pythonInterpreter.exec(
            String.format("%s=%s(%s)", operatorObjectName, classNamePath,
                paramToPythonString(config.getParams())
            )
        );
    ...
    private String paramToPythonString(Map<String, String> params) {
        ...
        String keyValue = params.entrySet().stream()
