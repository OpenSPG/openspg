...
/**
 * Reasoning based on KGDSL. This class contains a KGDSL script and possible starting vertices. When there is a starting
 * vertex, the task starts from that starting point for reasoning. If there is no starting vertex, the task will trigger
 * reasoning from a batch of vertices of the same type.
 */
public class KgdslReasonerContent extends BaseReasonerContent {
     ...
     * <p>
     * If it is empty, it usually consumes more resources, it is recommended to run in cluster mode.
     */
    private final List<StartingVertex> startingVertices;

    public KgdslReasonerContent(
        String kgdsl, List<StartingVertex> startingVertices) {
