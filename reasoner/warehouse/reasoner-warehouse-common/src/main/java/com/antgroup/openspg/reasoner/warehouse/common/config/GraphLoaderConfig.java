...
public class GraphLoaderConfig implements Serializable {
        ...
        if (vertexLoaderConfigs == null) {
            ...
            for (VertexLoaderConfig vertexLoaderConfig : other.vertexLoaderConfigs) {
                ...
                if (vertexLoaderConfigMap.containsKey(vType)) {
                    vertexLoaderConfigMap.put(vType, vertexLoaderConfigMap.get(vType).merge(vertexLoaderConfig));
                } else {
    ...
    @Override
    public int hashCode() {
        List<String> vertexTypeList = vertexLoaderConfigs.stream().map(VertexLoaderConfig::getVertexType).sorted().collect(
                Collectors.toList());
        List<String> edgeTypeList = edgeLoaderConfigs.stream().map(EdgeLoaderConfig::getEdgeType).sorted().collect(Collectors.toList());
    ...
    public void toJavaConfig() {
        this.vertexLoaderConfigs.forEach(new Consumer<VertexLoaderConfig>() {
            @Override
            public void accept(VertexLoaderConfig vertexLoaderConfig) {
                vertexLoaderConfig.setNeedProperties(Sets.newHashSet(vertexLoaderConfig.getNeedProperties()));
                vertexLoaderConfig.setPropertiesFilterRules(Lists.newArrayList(vertexLoaderConfig.getPropertiesFilterRules()));
            }
        });
        this.vertexLoaderConfigs = Sets.newHashSet(this.vertexLoaderConfigs);

        this.edgeLoaderConfigs.forEach(new Consumer<EdgeLoaderConfig>() {
            @Override
            public void accept(EdgeLoaderConfig edgeLoaderConfig) {
                edgeLoaderConfig.setNeedProperties(Sets.newHashSet(edgeLoaderConfig.getNeedProperties()));
                edgeLoaderConfig.setPropertiesFilterRules(Lists.newArrayList(edgeLoaderConfig.getPropertiesFilterRules()));
            }
        });
    ...
    public String toString() {
        StringBuilder str = new StringBuilder(
                "GraphLoaderConfig,schemaUrl=" + this.schemaUrl + ",binary=" + this.binary + ",edgeTruncateThreshold="
