...
public class WareHouseUtils {
    ...
    public static List<String> getVertexRuleList(Pattern pattern) {
        ...
        if (null != pattern.root().rule()) {
            vertexRuleList.addAll(JavaConversions.asJavaCollection(EXPR_TRANSFORMER.transform(pattern.root().rule())));
        }
    ...
    public static Map<String, List<String>> getDstVertexRuleList(Pattern pattern) {
        ...
        for (Connection connection : JavaConversions.setAsJavaSet(connectionSet.get())) {
            ...
            if (null != patternElement.rule()) {
                result.computeIfAbsent(dstAlias, k -> Lists.newArrayList(
                        JavaConversions.asJavaCollection(EXPR_TRANSFORMER.transform(patternElement.rule()))));
            }
        }
        return result;
    }
    ...
    public static Map<Connection, List<String>> getEdgeRuleMap(Pattern pattern) {
        ...
        if (!patternConnectionSet.isEmpty()) {
            for (Connection patternConnection : JavaConversions.setAsJavaSet(patternConnectionSet.get())) {
                if (null == patternConnection.rule()) {
                    continue;
                }
                List<String> edgeRuleList = edgeRuleMap.computeIfAbsent(patternConnection, k -> new ArrayList<>());
                edgeRuleList.addAll(JavaConversions.asJavaCollection(EXPR_TRANSFORMER.transform(patternConnection.rule())));
            }
        }
        return edgeRuleMap;
    }
        ...
        if (!patternConnectionSet.isEmpty()) {
            for (Connection patternConnection : JavaConversions.setAsJavaSet(patternConnectionSet.get())) {
                ...
                for (String edgeType : JavaConversions.setAsJavaSet(patternConnection.relTypes())) {
                    List<Rule> ruleList = edgeTypeRuleListMap.computeIfAbsent(edgeType, k -> new ArrayList<>());
    ...
    public static Tuple2<String, List<String>> getRuleListWithAlias(Rule rule) {
        List<String> ruleList = Lists.newArrayList(JavaConversions.asJavaIterable(EXPR_TRANSFORMER.transform(rule)));
        List<IRField> irFieldList = JavaConversions.seqAsJavaList(RuleUtils.getAllInputFieldInRule(rule, null, null));
