...
public class LocalThinkerMain {
    ...
    public static void doMain(String[] args) {
        ...
        LogicCatalog logicCatalog = new OpenSPGLogicCatalog(task.getProjectId(),
            task.getConnInfo());
        logicCatalog.init();
        Thinker thinker = new DefaultThinker(graphState, logicCatalog);
        List<Result> result;
        if (task.getMode().toLowerCase().equals("spo")) {
            result = thinker.find(task.getTriple().getSubject(),
                    task.getTriple().getPredicate(), task.getTriple().getObject(), task.getParams());
        } else {
    ...
    private static ThinkerParams parseArgs(String[] args) {
        ...
        try {
            ...
            if (s == Element.ANY && p == Element.ANY && o == Element.ANY) {
                throw new RuntimeException("subject, predicate, object cannot all be empty at the same time.");
            }
    ...
    private static Options getThinkerOptions() {
        ...
        options.addRequiredOption(ParamsKey.PROJECT_ID_OPTION, ParamsKey.PROJECT_ID_OPTION, true,
            "project id");
        options.addRequiredOption(ParamsKey.SCHEMA_URL_OPTION, ParamsKey.SCHEMA_URL_OPTION, true,
            "schema url");
        options.addOption(ParamsKey.SUBJECT, ParamsKey.SUBJECT, true,
            "query subject, eg: id,type or type");
        options.addOption(ParamsKey.PREDICATE, ParamsKey.PREDICATE, true,
            "query predicate, eg: type");
        options.addOption(ParamsKey.OBJECT, ParamsKey.OBJECT, true,
            "query object, eg: id,type or type");
        options.addOption(ParamsKey.OUTPUT_OPTION, ParamsKey.OUTPUT_OPTION, true,
            "output file name");
        options.addOption(ParamsKey.GRAPH_STATE_CLASS_OPTION, ParamsKey.GRAPH_STATE_CLASS_OPTION,
            true, "graph state class name");
        options.addOption(ParamsKey.GRAPH_LOADER_CLASS_OPTION, ParamsKey.GRAPH_LOADER_CLASS_OPTION,
            true, "graph loader class name");
        options.addOption(ParamsKey.GRAPH_STORE_URL_OPTION,
            ParamsKey.GRAPH_STORE_URL_OPTION, true, "graph store url");
        options.addOption(ParamsKey.PARAMs_OPTION, ParamsKey.PARAMs_OPTION, true, "params");
        options.addOption(ParamsKey.LOG_FILE_OPTION, ParamsKey.LOG_FILE_OPTION, true,
            "log file name");
    ...
    protected static GraphState<IVertexId> loadGraph(ThinkerParams params) {
        ...
        if (StringUtils.isNotEmpty(graphStateClass)) {
            try {
                graphState = (GraphState<IVertexId>) Class.forName(graphStateClass)
                    .getConstructor(String.class).newInstance(params.getGraphStateInitString());
            } catch (Exception e) {
        ...
        try {
            graphLoader = (AbstractLocalGraphLoader) Class.forName(graphLoadClass).getConstructor()
                .newInstance();
        } catch (Exception e) {
            throw new RuntimeException("can not create graph loader from name " + graphLoadClass,
                e);
        }
