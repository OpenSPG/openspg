...
public class ReadRangeTest {
  ...
  public void testReadRange1() {
    ...
    for (OdpsTableInfo partition : partitionCountMap.keySet()) {
      Assert.assertEquals(
          partitionCountMap.get(partition).longValue(), rangeMap.get(partition).getCount());
    }
  }
  ...
  public void testReadRange2() {
    ...
    for (int i = 0; i < parallel; ++i) {
      for (int j = 0; j < allRound; ++j) {
        Map<OdpsTableInfo, ReadRange> rangeMap =
            OdpsUtils.getReadRange(parallel, i, allRound, j, partitionCountMap);
        for (OdpsTableInfo tableInfo : rangeMap.keySet()) {
          List<ReadRange> readRangeList =
              resultRangeMap.computeIfAbsent(tableInfo, k -> new ArrayList<>());
