...
/**
 * GraphState based on Rocksdb
 * vertex key is V_TYPEID_ID_PLACEHOLDER_WINDOWS, value is vertex property
 * edge key is E_EDGETYPE_DIRECTION_VERTEXID_WINDOW, value is edges
 * @author kejian
...
public class RocksdbGraphState implements GraphState<IVertexId> {
    ...
    private Tuple2<byte[], IProperty> wrapVertex(IVertex<IVertexId, IProperty> vertex, long window) {
        return new Tuple2<>(RocksDBUtil.rocksdbKeyAppendWindow(
                RocksDBUtil.buildRocksDBVertexKeyWithoutWindow(vertex.getId()),
                window),
                vertex.getValue());
    }
     ...
     */
    @Override
    public void mergeVertexProperty(IVertexId id, Map<String, Object> property, MergeTypeEnum mergeType, Long version) {
        ...
        Tuple2<byte[], IProperty> wrapResult = wrapVertex(newVertex, this.helper.getWriteWindow(version));
        writeData(wrapResult._1, wrapResult._2);
    }
    ...
    private Map<String, TreeMap<Long, Object>> collapseVertexProperty(List<byte[]> propertyByteList) {
        List<IVersionProperty> vertexVersionProperties = propertyByteList.stream()
                .map(bytes -> {
        ...
        for (IVersionProperty property : vertexVersionProperties) {
            for (String key : property.getKeySet()) {
                TreeMap<Long, Object> thisVersionValueMap = resultMap.computeIfAbsent(key, k -> new TreeMap<>());
     ...
     * @param id
     * @param version if null return vertex with the all version properties, if Long.MAX_VALUE return newest properties
     * @return
     */
    @Override
     ...
     * @return
     */
    private List<Tuple2<byte[], IProperty>> wrapEdge(List<IEdge<IVertexId, IProperty>> edges, Direction direction, IVertexId vertexId) {
        List<Tuple2<byte[], IProperty>> wrapEdgeList = new ArrayList<>();
        Map<String, Map<Long, List<IEdge<IVertexId, IProperty>>>> type2Version2EdgeMap = new HashMap<>();
        for (IEdge<IVertexId, IProperty> edge : edges) {
            String p = edge.getType();
            Map<Long, List<IEdge<IVertexId, IProperty>>> version2EdgeListMap = type2Version2EdgeMap.computeIfAbsent(p,
                    k -> new HashMap<>());
            List<IEdge<IVertexId, IProperty>> edgeList = version2EdgeListMap.computeIfAbsent(
                    this.helper.getWriteWindow(edge.getVersion()), k -> new ArrayList<>());
            edgeList.add(edge);
        }
        for (String type : type2Version2EdgeMap.keySet()) {
            Map<Long, List<IEdge<IVertexId, IProperty>>> version2EdgeListMap = type2Version2EdgeMap.get(type);
            byte[] edgeKeyPrefix = RocksDBUtil.buildRocksDBEdgeKeyWithoutWindow(type, direction, vertexId);
     ...
     */
    @Override
    public void addEdges(IVertexId vertexId, List<IEdge<IVertexId, IProperty>> inEdges, List<IEdge<IVertexId, IProperty>> outEdges) {
        if (CollectionUtils.isNotEmpty(outEdges)) {
            wrapEdge(outEdges, Direction.OUT, vertexId).forEach(tuple -> writeData(tuple._1(), tuple._2()));
        }
     ...
     */
    @Override
    public void updateEdgeProperty(IVertexId s, String p, IVertexId o, Long version, IProperty property) {
     ...
     */
    @Override
    public void mergeEdgeProperty(IVertexId s, String p, IVertexId o, Long version, Direction direction, Map<String, Object> property,
     ...
     */
    @Override
    public List<IEdge<IVertexId, IProperty>> getEdgesWithoutProperty(IVertexId vertexId, Long startVersion, Long endVersion,
                                                                     Set<String> types,
                                                                     Direction direction) {
     ...
     */
    @Override
    public List<IEdge<IVertexId, IProperty>> getEdges(IVertexId vertexId, Long startVersion, Long endVersion, Set<String> types,
                                                      Direction direction) {
    ...
    @Override
    public List<IEdge<IVertexId, IProperty>> getEdges(IVertexId vertexId, Long startVersion, Long endVersion, Set<String> types,
                                                      Direction direction, Map<String, List<Rule>> typeAndRuleMap) {
     ...
     * @return
     */
    private List<IEdge<IVertexId, IProperty>> collectEdges(IVertexId vertexId, Long startVersion, Long endVersion, Set<String> types,
                                                           Direction direction) {
        List<IEdge<IVertexId, IProperty>> result = new ArrayList<>();
        for (String type : types) {
            byte[] rocksdbKeyPrefix = RocksDBUtil.buildRocksDBEdgeKeyWithoutWindow(type, direction, vertexId);
            List<byte[]> bytesList = readData(rocksdbKeyPrefix, this.helper.mapVersion2WindowRange(startVersion, endVersion));
            if (CollectionUtils.isEmpty(bytesList)) {
                continue;
            }
            List<Map<String, IEdge<IVertexId, IProperty>>> edgeList = bytesList.stream().map(bytes -> {
                Object obj = helper.byte2Object(bytes);
                if (!(obj instanceof EdgeProperty)) {
                    throw new RuntimeException("The data that getEdges reads from rocksdb is not EdgeProperty type");
                }
                if (!((EdgeProperty) obj).isKeyExist("edges")) {
                    throw new RuntimeException("getEdges EdgeProperty not contain edges");
                }
                List<IEdge<IVertexId, IProperty>> edges = ((List<IEdge<IVertexId, IProperty>>) ((EdgeProperty) obj).get("edges"));
        ...
        return result.stream().filter(e -> RunnerUtil.between(startVersion, endVersion, e.getVersion())).collect(Collectors.toList());
    }
     ...
     * @param newEdgeMap
     */
    private void mergeEdgeMap(Map<String, IEdge<IVertexId, IProperty>> oldEdgeMap, Map<String, IEdge<IVertexId, IProperty>> newEdgeMap) {
    ...
    public Iterator<IVertex<IVertexId, IProperty>> getVertexIterator(Set<String> vertexType) {
        return getVertexIterator(vertex -> {
    ...
    public Iterator<IVertex<IVertexId, IProperty>> getVertexIterator(Predicate<IVertex<IVertexId, IProperty>> filter) {
        ...
        return new Iterator<IVertex<IVertexId, IProperty>>() {
            ...
            private IVertex<IVertexId, IProperty> getNext() {
                ...
                IProperty vertexProperty = PropertyUtil.buildVertexProperty(vertexId, collapseVertexProperty(curVertexPropertyList));
