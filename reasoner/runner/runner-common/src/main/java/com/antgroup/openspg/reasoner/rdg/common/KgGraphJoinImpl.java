...
public class KgGraphJoinImpl implements Serializable {
    ...
    for (KgGraph<IVertexId> leftKgGraph : left) {
      ...
      for (KgGraph<IVertexId> rightKgGraph : right) {
        ...
        if (!leftKgGraph
            ...
            .get(0)
            .getId()
            .equals(rightKgGraph.getVertex(rightAlias).get(0).getId())) {
          // id not equals
          continue;
        }
        ...
        for (Map.Entry<String, String> entry : this.otherJoinAliasMap.entrySet()) {
          if (!leftKgGraph
              .getVertex(entry.getKey())
              .get(0)
              .getId()
              .equals(rightKgGraph.getVertex(entry.getValue()).get(0).getId())) {
            // id not equals
  ...
  private void leftJoinNone(KgGraphImpl kgGraph) {
    ...
    for (Map.Entry<Var, Var> entry :
        JavaConversions.mapAsJavaMap(this.rhsSchemaMapping).entrySet()) {
      if (entry.getKey() instanceof NodeVar) {
        ...
        throw new RuntimeException(
            "unsupported alias mapping type, " + entry.getKey().getClass().getName());
      }
    }
    for (Connection connection : RunnerUtil.getConnectionSet(this.rightSchema)) {
      Tuple2<String, IVertex<IVertexId, IProperty>> sourceTuple2 =
          getNoneVertex(connection.source(), rightVertexAliasMap, kgGraph);
      Tuple2<String, IVertex<IVertexId, IProperty>> targetTuple2 =
          getNoneVertex(connection.target(), rightVertexAliasMap, kgGraph);
      ...
      kgGraph
          .getAlias2EdgeMap()
          .put(
              rightEdgeAliasMap.get(connection.alias()),
              Sets.newHashSet(
                  new OptionalEdge<>(sourceTuple2._2().getId(), targetTuple2._2().getId())));
    }
  }

  private Tuple2<String, IVertex<IVertexId, IProperty>> getNoneVertex(
      String rightRowAlias, Map<String, String> rightVertexAliasMap, KgGraphImpl kgGraph) {
    ...
    if (otherJoinAliasReverseMap.containsKey(rightRowAlias)) {
      return new Tuple2<>(
          null,
          new NoneVertex<>(kgGraph.getVertex(otherJoinAliasReverseMap.get(rightRowAlias)).get(0)));
    }
    return new Tuple2<>(
        rightVertexAliasMap.get(rightRowAlias),
        new NoneVertex<>(kgGraph.getVertex(this.leftAlias).get(0)));
  }
}
