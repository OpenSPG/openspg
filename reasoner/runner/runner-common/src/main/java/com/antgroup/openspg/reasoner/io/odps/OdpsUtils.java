...
public class OdpsUtils {
    ...
    public static boolean isSchemaMatch(OdpsTableInfo odpsTableInfo, TableSchema realSchema) {
        ...
        List<String> columnsNeeded = schemaNeeded.getPartitionColumns().stream().map(com.aliyun.odps.Column::getName)
                .sorted(String::compareTo).collect(Collectors.toList());

        List<String> columnsReal = realSchema.getPartitionColumns().stream().map(com.aliyun.odps.Column::getName)
                .sorted(String::compareTo).collect(Collectors.toList());

        if (!columnsNeeded.equals(columnsReal)) {
            log.error("odps_partition_columns_not_match, need_columns=" + JSON.toJSONString(columnsNeeded)
                    + ",real_columns=" + JSON.toJSONString(columnsReal));
            return false;
        }

        columnsNeeded = schemaNeeded.getColumns().stream().map(com.aliyun.odps.Column::getName).sorted(
                String::compareTo).collect(Collectors.toList());

        columnsReal = realSchema.getColumns().stream().map(com.aliyun.odps.Column::getName).sorted(String::compareTo)
                .collect(Collectors.toList());
        if (!columnsNeeded.equals(columnsReal)) {
            log.error("odps_type_not_match, need_columns=" + JSON.toJSONString(columnsNeeded)
                    + ",real_columns=" + JSON.toJSONString(columnsReal));
            return false;
        }

        List<OdpsType> neededType = schemaNeeded.getColumns().stream().map(column -> column.getTypeInfo().getOdpsType())
                .sorted(Enum::compareTo).collect(Collectors.toList());

        List<OdpsType> realType = realSchema.getColumns().stream().map(column -> column.getTypeInfo().getOdpsType())
                .sorted(Enum::compareTo).collect(Collectors.toList());

        if (!neededType.equals(realType)) {
            log.error("odps_type_not_match, need_type=" + JSON.toJSONString(neededType)
                    + ",real_type=" + JSON.toJSONString(realType));
            return false;
        }
    ...
    public static void createTable(OdpsTableInfo odpsTableInfo) {
        ...
        try {
            odps.tables().createTableWithLifeCycle(odpsTableInfo.getProject(), odpsTableInfo.getTable(),
                    schema, null, true, 365L);
            log.info("create table success," + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable());
     ...
     * get upload session
     */
    public static UploadSession tryGetUploadSession(OdpsTableInfo odpsTableInfo, String id, int index, int parallel) {
        Odps odps = getODPSInstance(odpsTableInfo);
        ...
        while (--maxTryTimes >= 0) {
            try {
                return tunnel.getUploadSession(odpsTableInfo.getProject(), odpsTableInfo.getTable(),
                        getOdpsPartitionSpec(odpsTableInfo), id, parallel, index);
            } catch (Throwable e) {
     ...
     */
    public static DownloadSession tryCreateDownloadSession(TableTunnel tunnel, OdpsTableInfo odpsTableInfo) {
        ...
        while (--maxTryTimes >= 0) {
            try {
                ...
                if (null != partition) {
                    downloadSession = tunnel.createDownloadSession(odpsTableInfo.getProject(), odpsTableInfo.getTable(), partition);
                } else {
                    downloadSession = tunnel.createDownloadSession(odpsTableInfo.getProject(), odpsTableInfo.getTable());
                }
                return downloadSession;
            } catch (TunnelException e) {
                if ("NoSuchPartition".equals(e.getErrorCode())) {
                    // continue
                    log.info("table=" + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable()
                            + ", partition=" + partition + ", not exist");
                    return null;
                } else if ("InvalidPartitionSpec".equals(e.getErrorCode())) {
                    // if this table is not a partition table, we create download session without PartitionSpec
                    log.info("table=" + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable()
                            + ", partition=" + partition + ", InvalidPartitionSpec");
                    partition = null;
                    continue;
                ...
                log.error("create_download_session_error, table=" + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable()
        ...
        throw new OdpsException("create_download_session_failed, time_out, table="
                + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable()
                + ", partition=" + partition, lastError);
    }

    /**
     * open record reader
     */
    public static TunnelRecordReader tryOpenRecordReader(TableTunnel.DownloadSession downloadSession, long start, long count) {
    ...
    public static UploadSession createUploadSession(OdpsTableInfo odpsTableInfo) {
        ...
        if (null == schema) {
            if (odpsTableInfo.getProject().endsWith("_dev")) {
                ...
                // table not exist
                throw new OdpsException("table not exist, project="
                        + odpsTableInfo.getProject() + ",table=" + odpsTableInfo.getTable()
                        , null);
            }
        }
        ...
        if (!isSchemaMatch(odpsTableInfo, schema)) {
            throw new OdpsException("table " + odpsTableInfo.getProject() + "." + odpsTableInfo.getTable()
                    + ",schema not match", null);
        }
     ...
     * get read range
     */
    public static Map<OdpsTableInfo, ReadRange> getReadRange(int parallel, int index, int allRound, int nowRound,
                                                             Map<OdpsTableInfo, Long> tableCountMap) {
        ...
        tableCountList.sort(Comparator.comparingLong((Pair<OdpsTableInfo, Long> o) -> o.getRight()).thenComparing(Pair::getLeft));
        ...
        result = result.entrySet().stream().filter(tableReadRange -> tableReadRange.getValue().getCount() > 0)
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

        return result;
    }

    private static ReadRange getReadRange(int parallel, int index, int allRound, int nowRound, long count,
                                          long minLoadSize) {
