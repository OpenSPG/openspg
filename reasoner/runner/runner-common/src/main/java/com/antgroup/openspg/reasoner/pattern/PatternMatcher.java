...
public class PatternMatcher implements Serializable {
     ...
     * @return
     */
    public KgGraph<IVertexId> patternMatch(IVertexId id, Long startVersion, Long endVersion,
                                           Pattern pattern,
                                           ...
                                           Rule vertexRule, Map<String, List<Rule>> edgeTypeRuleMap,
                                           Long limit, boolean enableStarPathLimit,
                                           long timeoutMillis) {
        ...
        // start match edges
        Option<scala.collection.immutable.Set<Connection>> patternConnectionSet = pattern.topology().get(pattern.root().alias());
        ...
        Map<String, Map<Direction, ArrayList<IEdge<IVertexId, IProperty>>>> edgeTypeDirectionMap = new TreeMap<>();
        for (Connection patternConnection : patternConnections) {
            ...
            for (String edgeType : edgeSpoTypeList) {
                Map<Direction, ArrayList<IEdge<IVertexId, IProperty>>> directionArrayListMap = edgeTypeDirectionMap.computeIfAbsent(
                        edgeType, k -> new TreeMap<>());
        ...
        for (String edgeType : edgeTypeDirectionMap.keySet()) {
            Map<Direction, ArrayList<IEdge<IVertexId, IProperty>>> directionArrayListMap = edgeTypeDirectionMap.get(edgeType);
            for (Direction direction : directionArrayListMap.keySet()) {
                ...
                List<IEdge<IVertexId, IProperty>> edges = graphState.getEdges(id, startVersion, endVersion,
                        Sets.newHashSet(edgeType), direction, edgeTypeRuleMap);
        ...
        for (Connection patternConnection : patternConnections) {
            ...
            if (null != validVertexIdSet) {
                willMatchEdgeList.removeIf(new Predicate<IEdge<IVertexId, IProperty>>() {
                    @Override
            ...
            List<IEdge<IVertexId, IProperty>> validEdges = matchEdges(vertexContext, willMatchEdgeList, patternConnection, pattern,
                    edgeRuleMap, limit);
    ...
    private List<IEdge<IVertexId, IProperty>> matchEdges(Map<String, Object> vertexContext,
        ...
        for (IEdge<IVertexId, IProperty> edge : edgeList) {
            if (!isEdgeMatch(vertexContext, edge, patternConnection, pattern,
                    edgeRuleMap.get(patternConnection))) {
                continue;
            }
            oneTypeEdgeCount++;
    ...
    private boolean isEdgeMatch(Map<String, Object> vertexContext, IEdge<IVertexId, IProperty> edge,
                                Connection patternConnection,
                                Pattern pattern,
        ...
        if (!pattern.getNode(patternConnection.target()).typeNames().contains(RunnerUtil.getEdgeTargetType(edge))) {
            return false;
        }

        // edge source type match
        if (!pattern.getNode(patternConnection.source()).typeNames().contains(RunnerUtil.getEdgeSourceType(edge))) {
            return false;
        }

        // edge rule match
        if (null != edgeRuleList && !edgeRuleList.isEmpty()) {
            Map<String, Object> edgeContext = RunnerUtil.edgeContext(edge, spo, patternConnection.alias());
    ...
    private List<String> getEdgeSpoTypeList(Connection patternConnection, Pattern pattern) {
        ...
        Set<String> pSet = Sets.newHashSet(JavaConversions.asJavaCollection(patternConnection.relTypes()));
        ...
        Set<String> sSet = Sets.newHashSet(JavaConversions.asJavaCollection(pattern.getNode(sourceAlias).typeNames()));
        Set<String> oSet = Sets.newHashSet(JavaConversions.asJavaCollection(pattern.getNode(targetAlias).typeNames()));
