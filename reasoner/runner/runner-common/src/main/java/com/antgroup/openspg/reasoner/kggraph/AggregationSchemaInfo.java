...
public class AggregationSchemaInfo implements Serializable {
  ...
  public AggregationSchemaInfo(Pattern schema) {
    for (Connection pc : RunnerUtil.getConnectionSet(schema)) {
      edgeEndpointMap.put(
          pc.alias(),
          Lists.newArrayList(new Tuple2<>(pc.source(), true), new Tuple2<>(pc.target(), false)));
      List<Tuple2<String, Boolean>> sourceEdgeInfoList =
          vertexHasEdgeMap.computeIfAbsent(pc.source(), k -> new ArrayList<>());
      sourceEdgeInfoList.add(new Tuple2<>(pc.alias(), true));
      List<Tuple2<String, Boolean>> targetEdgeInfoList =
          vertexHasEdgeMap.computeIfAbsent(pc.target(), k -> new ArrayList<>());
