...
private static final long serialVersionUID = -3632032496948942544L;
    ...
    public FoldEdgeImpl(PartialGraphPattern kgGraphSchema, FoldRepeatEdgeInfo foldRepeatEdgeInfo) {
        for (Connection connection : RunnerUtil.getConnectionSet(kgGraphSchema)) {
            ...
            noneMatchRepeatFoldInfo = new FoldRepeatEdgeInfo(pathConnection.alias(), pathConnection.alias(),
                    pathConnection.source(), pathConnection.target());
        }
        this.foldRepeatEdgeInfo = foldRepeatEdgeInfo;
    }

    private void foldNone(KgGraphImpl kgGraph) {
        Set<IVertex<IVertexId, IProperty>> vertexSet = kgGraph.getAlias2VertexMap().get(this.noneMatchRepeatFoldInfo.getFromVertexAlias());
    ...
    public List<KgGraph<IVertexId>> fold(KgGraphImpl kgGraph) {
        ...
        Set<IEdge<IVertexId, IProperty>> pathEdgeSet = kgGraph.getAlias2EdgeMap().get(this.foldRepeatEdgeInfo.getToEdgeAlias());

        Set<IEdge<IVertexId, IProperty>> fromEdgeSet = kgGraph.getAlias2EdgeMap().remove(this.foldRepeatEdgeInfo.getFromEdgeAlias());
        if (null == fromEdgeSet || fromEdgeSet.isEmpty()) {
            // repeat match nothing
            Set<IVertex<IVertexId, IProperty>> toVertexSet = kgGraph.getAlias2VertexMap().get(this.foldRepeatEdgeInfo.getToVertexAlias());
            if (null == toVertexSet) {
                foldNone(kgGraph);
            }
            result.add(kgGraph);
            ...
            if (edge instanceof OptionalEdge) {
                ...
                Set<IVertex<IVertexId, IProperty>> fromVertexSet = kgGraph.getAlias2VertexMap().remove(
                        this.foldRepeatEdgeInfo.getFromVertexAlias());
        ...
        if (null == pathEdgeSet) {
            ...
            for (IEdge<IVertexId, IProperty> edge : fromEdgeSet) {
                PathEdge<IVertexId, IProperty, IProperty> pathEdge = new PathEdge<>((Edge<IVertexId, IProperty>) edge);
                toEdgeSet.add(pathEdge);
            }
            kgGraph.getAlias2EdgeMap().put(this.foldRepeatEdgeInfo.getToEdgeAlias(), toEdgeSet);

            // 重命名点
            kgGraph.getAlias2VertexMap().put(this.foldRepeatEdgeInfo.getToVertexAlias(),
                    kgGraph.getAlias2VertexMap().remove(this.foldRepeatEdgeInfo.getFromVertexAlias()));
            result.add(kgGraph);
            return result;
        }
        ...
        for (IEdge<IVertexId, IProperty> edge : pathEdgeSet) {
            PathEdge<IVertexId, IProperty, IProperty> pathEdge = (PathEdge<IVertexId, IProperty, IProperty>) edge;
            IVertexId pathEdgeSearchId = pathEdge.getTargetId();
            Vertex<IVertexId, IProperty> nextVertex = (Vertex<IVertexId, IProperty>) toVertexMap.get(pathEdgeSearchId);
            ...
            for (IEdge<IVertexId, IProperty> nextIEdge : nextEdgeList) {
                ...
                PathEdge<IVertexId, IProperty, IProperty> newPathEdge = new PathEdge<>(pathEdge, nextVertex, nextEdge);
