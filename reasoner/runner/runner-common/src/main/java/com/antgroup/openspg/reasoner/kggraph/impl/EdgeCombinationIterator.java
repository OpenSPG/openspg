...
/**
 * This class implements an iterator that combines edges into a path.
 * taking as input a map of schema and edges.
 *
 * example1:
 * schema is [A -E1-> B, B -E2-> C], edgeMap is {E1:[a1_b1, a1_b2], E2:[b1_c1, b1_c2]}
...
public class EdgeCombinationIterator implements Iterator<KgGraph<IVertexId>> {
    ...
    public EdgeCombinationIterator(List<EdgeIterateInfo> edgeIterateInfoList, Map<String, Integer> edgeIterateOrderMap,
    ...
    private void pushNextIterator() {
        ...
        List<IEdge<IVertexId, IProperty>> edgeList = aliasEdgeQueryMap.get(buildEdgeIteratorInfo.getQueryEdgeAlias()).get(queryKey);
    ...
    private boolean checkIntersectionAndDuplicateVertex(EdgeIterateInfo edgeIterateInfo,
        ...
        if (!intersectInfoList.isEmpty()) {
            ...
            for (IntersectVertexInfo intersectVertexInfo : intersectInfoList) {
                IEdge<IVertexId, IProperty> checkEdge = resultEdgeStack.get(intersectVertexInfo.getCheckVertexInfo().getEdgeIndex());
                IVertexId checkId;
        ...
        Map<Integer, Tuple2<Boolean, Boolean>> duplicateVertexCheckMap = edgeIterateInfo.getDuplicateVertexCheck();
        if (MapUtils.isNotEmpty(duplicateVertexCheckMap)) {
            for (Map.Entry<Integer, Tuple2<Boolean, Boolean>> entry : duplicateVertexCheckMap.entrySet()) {
        ...
        try {
            ...
            for (int i = 0; i < this.edgeIterateInfoList.size(); ++i) {
                ...
                if (!alias2VertexMap.containsKey(sourceAlias)) {
                    IVertex<IVertexId, IProperty> v = this.alias2VertexMap.get(sourceAlias).get(edge.getSourceId());
                ...
                if (!alias2VertexMap.containsKey(targetAlias)) {
                    IVertex<IVertexId, IProperty> v = this.alias2VertexMap.get(targetAlias).get(edge.getTargetId());
            ...
            if (nextCount++ > logCount) {
                ...
                for (int i = 0; i < this.edgeIterateInfoList.size(); ++i) {
                    ...
                    sb.append(",").append(i).append(",e=").append(edgeIterateInfo1.getEdgeAlias()).append(",s=").append(
                            edgeIterateInfo1.getSourceAlias()).append(",o=").append(edgeIterateInfo1.getTargetAlias());
                }
                sb.append("\nVertexId");
    ...
    private Map<String, Map<IVertexId, List<IEdge<IVertexId, IProperty>>>> initEdgeQueryMap(
            Map<String, Set<IEdge<IVertexId, IProperty>>> alias2EdgeMap, Map<String, Boolean> edgeIndexInfo) {
        ...
        for (Map.Entry<String, Set<IEdge<IVertexId, IProperty>>> entry : alias2EdgeMap.entrySet()) {
            ...
            Map<IVertexId, List<IEdge<IVertexId, IProperty>>> edgeMap = result.computeIfAbsent(edgeAlias, k -> new HashMap<>());
            for (IEdge<IVertexId, IProperty> edge : entry.getValue()) {
                ...
                List<IEdge<IVertexId, IProperty>> edgeList = edgeMap.computeIfAbsent(indexKey, k -> new ArrayList<>());
                edgeList.add(edge);
            }
        }
        return result;
    ...
    private Map<String, Map<IVertexId, IVertex<IVertexId, IProperty>>> initVertexQueryMap(
        ...
        for (Map.Entry<String, Set<IVertex<IVertexId, IProperty>>> entry : alias2VertexMap.entrySet()) {
            ...
            Map<IVertexId, IVertex<IVertexId, IProperty>> vertexMap = result.computeIfAbsent(vertexAlias, k -> new HashMap<>());
    ...
    private Map<String, Boolean> getEdgeIndexInfo() {
        ...
        for (EdgeIterateInfo edgeIterateInfo : this.edgeIterateInfoList) {
            ...
            edgeIndexInfo.put(buildEdgeIteratorInfo.getQueryEdgeAlias(), buildEdgeIteratorInfo.isIndexSourceOrTarget());
        }
    ...
    public static class BuildEdgeIteratorInfo implements Serializable {
        ...
        public BuildEdgeIteratorInfo(String queryEdgeAlias, boolean indexSourceOrTarget, Edge2VertexInfo edge2VertexInfo) {
