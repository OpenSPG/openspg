...
import com.antgroup.openspg.reasoner.thinker.logic.graph.NodeAny;
...
public class DefaultThinkerTests {
    private GraphState<IVertexId> buildGraphState() {
        ...
        Vertex<IVertexId, IProperty> vertex1 = new Vertex<>(IVertexId.from("a1", "A"),
            new VertexVersionProperty());
        Vertex<IVertexId, IProperty> vertex2 = new Vertex<>(IVertexId.from("a2", "A"),
            new VertexVersionProperty());
        Vertex<IVertexId, IProperty> vertex3 = new Vertex<>(IVertexId.from("b", "B"),
            new VertexVersionProperty());
        Vertex<IVertexId, IProperty> vertex4 = new Vertex<>(IVertexId.from("c", "C"),
            new VertexVersionProperty("k1", "abcd"));

        Edge a1b = new Edge(IVertexId.from("a1", "A"), IVertexId.from("b", "B"),
            new VertexVersionProperty(), 0L, Direction.OUT, "ab");
        Edge a2b = new Edge(IVertexId.from("a2", "A"), IVertexId.from("b", "B"),
            new VertexVersionProperty(), 0L, Direction.OUT, "ab");
        Edge bc = new Edge(IVertexId.from("b", "B"), IVertexId.from("c", "C"),
            new VertexVersionProperty(), 0L, Direction.OUT, "bc");
        ...
        graphState.addEdges(vertex3.getId(), Arrays.asList(a1b.reverse(), a2b.reverse()),
            Arrays.asList(bc));
    ...
    public void testFindForward() {
        ...
        List<Triple> triples = thinker.find(new Entity<>(IVertexId.from("a1", "A")),
            new NodeAny(), new NodeAny());
    ...
    public void testBackForward() {
        Thinker thinker = new DefaultThinker(buildGraphState());
        List<Triple> triples = thinker.find(new NodeAny(),
                new NodeAny(), new Entity<>(IVertexId.from("b", "B")));
